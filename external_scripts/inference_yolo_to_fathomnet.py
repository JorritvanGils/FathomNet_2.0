...
for filename in os.listdir(exp_dir):
    ...
    if os.path.getsize(os.path.join(exp_dir, filename)) != 0:
        ...
        with open(os.path.join(exp_dir, filename), "r") as label_file:
            for line in label_file:
                ...
                if yolo_pred[0] not in seen_classes.keys():
                    seen_classes[int(yolo_pred[0])+1] = float(yolo_pred[5])
                else:
                    seen_classes[int(yolo_pred[0])+1] = float(seen_classes[yolo_pred[0]]) + float(yolo_pred[5])
        ...
        submission_line.append(1-(sum(probs_of_bbs)/len(probs_of_bbs)))
    else:
